...
const SERVICES = [
  ...
  },
  {
    ...
    features: [
      ...
      'Múltiples secciones (Nosotros, Servicios, Galería, etc.)',
...
const colorMap: Record<string, { bg: string; border: string; text: string; badge: string; btn: string }> = {
  blue:   { bg: 'bg-blue-50',   border: 'border-blue-100',   text: 'text-blue-600',   badge: 'bg-blue-100 text-blue-700',   btn: 'bg-blue-600 hover:bg-blue-700' },
  indigo: { bg: 'bg-indigo-50', border: 'border-indigo-200', text: 'text-indigo-600', badge: 'bg-indigo-100 text-indigo-700', btn: 'bg-indigo-600 hover:bg-indigo-700' },
  violet: { bg: 'bg-violet-50', border: 'border-violet-100', text: 'text-violet-600', badge: 'bg-violet-100 text-violet-700', btn: 'bg-violet-600 hover:bg-violet-700' },
};

export const Services = ({ scrollTo }: { scrollTo: (id: string) => void }) => {
  return (
    <section id="servicios" className="py-20 sm:py-28 px-5 sm:px-8 bg-white">
            ...
            return (
              <motion.div
                ...
                className={`relative rounded-3xl border-2 p-7 flex flex-col ${
                  svc.popular
                    ? 'border-indigo-300 shadow-xl shadow-indigo-100/60 bg-gradient-to-b from-indigo-50/60 to-white'
              ...
              >
                ...
                <div className="flex items-baseline gap-1 mb-6">
                  ...
                  <span className="text-slate-400 text-sm font-medium">ARS</span>
                </div>
                ...
                <ul className="space-y-3 mb-8 flex-1">
                  {svc.features.map(f => (
                    <li key={f} className="flex items-start gap-2.5 text-sm text-slate-600">
                      <div className={`w-4 h-4 rounded-full ${c.bg} flex items-center justify-center shrink-0 mt-0.5`}>
                        <Check size={9} className={c.text} strokeWidth={3} />
                      </div>
                      {f}
                    </li>
                  ))}
