  ...
  return (
    <>
      <header
        ...
        style={{
          background: scrolled ? 'rgba(10,11,20,0.82)' : 'rgba(10,11,20,0.45)',
      ...
      >
        <div className="max-w-6xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-0.5">
            {DESKTOP_LINKS.map(l => (
              <button
                key={l.id}
                onClick={() => handle(l.id)}
                className="px-3.5 py-2 rounded-lg text-sm font-medium text-white/55 hover:text-white hover:bg-white/5 transition-all"
              >
                {l.label}
              </button>
            ))}
          ...
          <a
            ...
            className="hidden md:flex items-center gap-2 text-white font-semibold text-sm px-4 py-2 rounded-xl transition-all hover:opacity-90 active:scale-95"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ background: 'linear-gradient(160deg, #0a0b14 0%, #100a1c 100%)' }}
          >
            {/* Glow accent */}
            <div className="absolute top-0 right-0 w-72 h-72 rounded-full pointer-events-none opacity-10 blur-3xl"
              style={{ background: 'radial-gradient(circle, #f97316, transparent 70%)' }} />
            <div className="absolute bottom-20 left-0 w-48 h-48 rounded-full pointer-events-none opacity-8 blur-3xl"
            ...
            <nav className="flex flex-col px-4 pt-4 gap-0.5 flex-1 overflow-y-auto">
              {MOBILE_LINKS.map((l, i) => (
                <motion.button
                  ...
                  onClick={() => handle(l.id)}
                  className="flex items-center justify-between text-left px-4 py-3.5 rounded-xl text-white/55 hover:text-white active:text-white hover:bg-white/5 active:bg-white/5 transition-all group border border-transparent hover:border-white/5"
                >
                  <span className="text-lg font-bold tracking-tight">{l.label}</span>
                  <span className="font-mono text-xs text-white/20 group-hover:text-white/40 transition-colors">0{i + 1}</span>
                </motion.button>
              ))}
            </nav>
            ...
            >
              ...
              <a
                ...
                className="flex items-center justify-center gap-2 text-white/45 hover:text-white/70 text-sm transition-colors py-2"
              >
                <Mail size={15} />
